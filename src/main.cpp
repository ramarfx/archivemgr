...
void printUsage() {
  ...
  std::cout << "Usage:\n";
  std::cout << "  archivemgr create [-o <archive.zip>] [-p <password>] <file1> <file2> ...\n";
  std::cout << "  archivemgr list <archive.zip>\n";
  std::cout << "  archivemgr extract <archive.zip> [-o <output_dir>] [-p <password>]\n";
...
int main(int argc, char *argv[]) {
  ...
  if (command == "list") {
    ...
    int fileCount = std::count_if(entries.begin(), entries.end(),
                                  [](const FileEntry &e) { return !e.isDir; });
    ...
    if (largest != entries.end() && !largest->isDir) {
      std::cout << "Largest file: " << largest->name << " (" << largest->size
                << " bytes)\n";
    }
    ...
    for (int i = 3; i < argc; ++i) {
      ...
      if (arg == "-o") {
        ...
        if (i + 1 < argc) {
          ...
          std::cerr << "Error: -p flag requires a password.\n";
          return 1;
        }
      } else {
    ...
    extractZip(archivePath, outputDir, password);
    recordHistory("Extracted " + archivePath + " to " + outputDir + (password.empty() ? "" : " with password"));
    ...
    createZip(archivePath, files, password);
    recordHistory("Created archive " + archivePath + " with " +
                  std::to_string(files.size()) + " files" + (password.empty() ? "" : " and password"));
